...
opt.distributionSizes = {opt.distributionSizes};
if length(opt.distributionMarkers) == 1
    % expand
    opt.distributionSizes = repmat(opt.distributionSizes,nData,1);
end
...
if isempty(opt.categoryIdx)
    ...
    unique_categories = unique(opt.categoryIdx);
    
    nCategories = max(categoryIdx);
    nCategories = length(unique_categories);
...
plotSizes = repmat(opt.distributionSizes(:),1,nCategories);
...
if nCategories > 1
    ...
    if ~any(strcmp('categoryMarkers',parserObj.UsingDefaults))
        ...
        plotSizes = repmat(opt.categorySizes(:)',nData,1);
        categoryIsLabeled = true;
    end
            ...
            switch opt.xyOri
                case 'normal'
                    ph(iData,iCategory) = plot(ah,data(currentIdx,1),...
                        ...
                        'markers', plotSizes{iData,iCategory},... 
                        'color',plotColors{iData,iCategory},...
                        'lineStyle','none', 'linewidth',1,'MarkerFaceColor',plotColors{iData,iCategory},...
