...
baseline_correlations_stim = cell(2,2);
...
%     f = figure('Position',[800 200 800 800]);  hold on
    ...
    baseline_correlations_stim{1,site} = zeros(num_neurons,3);
    baseline_correlations_stim{1,site} = zeros(1,3);    
...
baseline_correlations_stim{2,site}(1) = W;    
baseline_correlations_stim{2,site}(2) = R(2);    
baseline_correlations_stim{2,site}(3) = P(2);  
...
disp(R(2));
    ...
    for unit_num = 1: num_neurons*analyze_single_units
        ...
        baseline_correlations_stim{1,site}(unit_num,1) = W;    
        baseline_correlations_stim{1,site}(unit_num,2) = R(2);   
        baseline_correlations_stim{1,site}(unit_num,3) = P(2);    
